...
public class CardController {
    ...
    public ResponseEntity<Void> newCardRequest(@RequestBody @NotNull @Valid NewCardRequest newCardRequest) {
        ...
        ClientInfoDto clientInfoDto = cardService.createCardRequest(mapToNewCardRequestDto(newCardRequest));

        log.info("newCardRequest: new card request stored for client, client ID: {}", clientInfoDto.getClient().getId());

        URI cardRequestUri = ServletUriComponentsBuilder.fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(clientInfoDto.getClient().getId())
                .toUri();
     ...
     *          {@linkplain ClientInfoDto} data in response body
     * @throws NotFoundException if response to be updated is not found
     */
    @PutMapping("/{clientId}/request/{requestId}")
    public ResponseEntity<ClientInfoDto> updateCardRequest(@PathVariable Long clientId,
        ...
        ClientInfoDto clientInfo = cardService.updateCardRequest(mapToCardRequestDto(clientId, requestId, updateCardRequest));
     ...
     */
    @GetMapping
    public ResponseEntity<ClientInfoDto> getClientInfo(@RequestParam @NotNull String oib) throws NotFoundException {
